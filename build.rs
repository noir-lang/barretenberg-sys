...
fn main() -> Result<()> {
    ...
    // Turned off multi-threading in BB so we can ignore libomp
    // link_lib_omp();

    println!(
        "cargo:rustc-link-search=native={}",
        "/mnt/user-data/kev/barretenberg-sys/static-libs"
    );

    // #[cfg(target_family = "unix")]
    // println!("cargo:rustc-link-lib=static=barretenberg-linux");
